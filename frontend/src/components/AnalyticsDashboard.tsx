...
import { TrendingDown, CreditCard, Clock, ShieldAlert, IndianRupee, AlertTriangle, BarChart3 } from 'lucide-react'
...
export function AnalyticsDashboard({ data }: { data: AnalyticsData }) {
  ...
  const rl = data.revenue_leakage
  const pm = data.payment_mode_profitability
  const sv = data.settlement_velocity
  const mr = data.merchant_risk_scores

  if (!rl || !pm || !sv || !mr) return null

  const modes = pm.modes ?? []
  const merchants = mr.merchants ?? []
  const histogram = sv.histogram ?? []
  const feeDetails = rl.fee_overcharge?.details ?? []

  const riskiestMerchant = merchants[0]
  const mostProfitable = pm.summary?.most_profitable_mode ?? 'N/A'
        ...
        <Card className="overflow-hidden">
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-lg font-semibold tabular-nums text-sky-400">{sv.avg_days ?? 0}d</p>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3 mb-4">
            <div className="rounded-lg border border-rose-500/10 bg-rose-500/5 p-3">
              ...
              <p className="text-lg font-semibold tabular-nums text-rose-400">{rl.fee_overcharge?.count ?? 0}</p>
            ...
            <div className="rounded-lg border border-amber-500/10 bg-amber-500/5 p-3">
              ...
              <p className="text-lg font-semibold tabular-nums text-amber-400">{rl.duplicate_settlements?.count ?? 0}</p>
            ...
            <div className="rounded-lg border border-orange-500/10 bg-orange-500/5 p-3">
              ...
              <p className="text-lg font-semibold tabular-nums text-orange-400">{rl.orphan_float?.count ?? 0}</p>
                ...
                <tbody className="text-zinc-400">
                  {feeDetails.map((d) => (
                    <tr key={d.razorpay_id} className="border-b border-white/[0.03]">
                      ...
                      <td className="py-1.5 text-right tabular-nums text-rose-400">{fmtINR(d.overcharge_amount)} ({d.overcharge_pct ?? 0}%)</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent>
            {modes.length > 0 ? (
              <>
                <div className="h-52">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={modes} layout="vertical" margin={{ left: 10, right: 20, top: 0, bottom: 0 }}>
              ...
              <p className="text-xs text-zinc-500 py-8 text-center">No payment mode data</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden">
          <CardHeader>
            ...
            <Badge tone="sky">{sv.delayed_count ?? 0} delayed</Badge>
          </CardHeader>
          <CardContent>
            {histogram.length > 0 ? (
              <>
                <div className="h-52">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={histogram} margin={{ left: -10, right: 10, top: 6, bottom: 0 }}>
                ...
                <div className="mt-3 flex gap-4 text-xs text-zinc-500">
                  <span>Avg: <span className="tabular-nums text-zinc-300">{sv.avg_days ?? 0}d</span></span>
                  <span>Median: <span className="tabular-nums text-zinc-300">{sv.median_days ?? 0}d</span></span>
                  <span>Max: <span className="tabular-nums text-zinc-300">{sv.max_days ?? 0}d</span></span>
                  <span>Delayed: <span className="tabular-nums text-amber-400">{sv.delayed_rate_pct ?? 0}%</span></span>
                </div>
              </>
            ) : (
              <p className="text-xs text-zinc-500 py-8 text-center">No velocity data</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent>
          {merchants.length > 0 ? (
                ...
                <tbody className="text-zinc-400">
                  {merchants.map((m) => (
                    <tr key={m.merchant} className="border-b border-white/[0.03]">
                      ...
                      <td className="py-1.5 pr-4 text-right tabular-nums">{m.breakdown?.orphan_rate_pct ?? 0}%</td>
                      <td className="py-1.5 pr-4 text-right tabular-nums">{m.breakdown?.fee_discrepancy_rate_pct ?? 0}%</td>
                      <td className="py-1.5 pr-4 text-right tabular-nums">{m.breakdown?.outlier_count ?? 0}</td>
                      <td className="py-1.5 text-right tabular-nums">{m.breakdown?.avg_settlement_delay_days ?? 0}d</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <p className="text-xs text-zinc-500 py-8 text-center">No merchant risk data</p>
          )}
        </CardContent>
      </Card>
    </div>
